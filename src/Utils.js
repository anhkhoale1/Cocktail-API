...
export const parseDrinks = (drinks) => {
  ...
  for (const apiCocktail of drinks) {
    ...
    const foundIndex = cocktails.findIndex((cocktail) => cocktail.idDrink === apiCocktail.idDrink);
    if (foundIndex === -1) {
      ...
      cocktails[foundIndex].ingredients = addIngredients(cocktails[foundIndex].ingredients, newIngredients);
...
function parseMeasureQuantity(strMeasure) { // 1 1/2 oz => 1,5 oz
  ...
  res.quantity = splittedMeasure.reduce((acc, val) => isNumber(val) ? eval(acc) + eval(val) : eval(acc), 0);
...
// [{ name: Yogurt Cooler, 
//   ingredients:[ {name : "Tequila",  measure: "3 oz" } , { name: ..., measure:...} ,...]
// }, 
// { name : Yellow Bird, 
// ingredients : [...]
// }]
